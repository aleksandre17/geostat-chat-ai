#!/usr/bin/env python3
"""Verify apps/backend matches restore-manifest.sha256 (byte-exact)."""
...
def main() -> int:
    if not MANIFEST.exists():
        ...
        print("Run: python tools/restore-backend-from-transcript.py", file=sys.stderr)
        return 1
    ...
    for line in MANIFEST.read_text(encoding="utf-8").splitlines():
        ...
        if sha256(content) != digest:
            bad.append(f"HASH_MISMATCH {rel}")
        else:
            ok += 1
